...
public class UpdateModel : Infrastructure.BasePageModelWithDatabaseContext
{
	...
	public async Task<IActionResult> OnPostAsync()
	{
		...
		try
		{
			...
			string? fixedTitle = NZLib.Utility.FixText(text: foundedItem.Title);
			...
			if (foundedAny)
			{
				var errorMessage = string.Format(Resources.Messages.Errors.AlreadyExists,
			...
			{
				bool isParent = await DatabaseContext.MenuItems
					.Where(current => current.ParentId == foundedItem.Id)
					.AnyAsync();

				if (isParent)
				{
					string errorMessage = Resources.Messages.Errors.UnableToUpdateParent;
					AddPageError(message: errorMessage);
			...
			var successMessage = string.Format(Resources.Messages.Successes.Updated,
