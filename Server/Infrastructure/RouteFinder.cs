...
{
	public static IList<ViewModels.Pages.Admin.ApplicationHandlers.CreateViewModel> Find()
	{
		var assembly = System.Reflection.Assembly.GetAssembly(type: typeof(Server.Pages.IndexModel));
		var pageModelType = typeof(PageModel);
		var compilerGeneratedAttributeType = typeof(System.Runtime.CompilerServices.CompilerGeneratedAttribute);
		...
		var foundedHandlers = routes?
			.Where(current => current.GetCustomAttributes(attributeType: compilerGeneratedAttributeType, inherit: true).Any() == false)
	...
	private static string GetPath(string fullName, string handler)
	{
		...
		{
			fullName = fullName.Remove(startIndex: fullName.Length - 5);
		}

		var path = fullName.Replace(oldValue: baseNamespace, newValue: string.Empty);
		path = path.Replace(oldValue: dot, newValue: slash);
		...
		{
			path = path.Replace(oldValue: doubleSlash, newValue: slash);
		}
